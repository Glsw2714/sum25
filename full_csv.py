...
- Generates a full suite of Matplotlib comparison plots, including y-cut resolutions.
- Creates multiple types of ROOT histograms and a TTree.
"""

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.colors import LogNorm # Import LogNorm for log scale color bars
import glob
import os
import re
import itertools
import ROOT # Import PyROOT

# --- Core Settings ---
data_base_dir = "/scratch/gregory/allcsv4"
base_output_dir = "/home/gregory/sum25/eic/analysis/all_method_plots2"
...
def save_2dhist(x, y, xlabel, ylabel, title, filename, target_dir, bins=100, use_percentiles=True, vmin=1, custom_range=None, text_to_display=None, x_scale='linear', y_scale='linear'):
    ...
    if custom_range:
        ...
        range_x = compute_range(x_filtered, use_percentiles=use_percentiles)
        range_y = compute_range(y_filtered, use_percentiles=use_percentiles)
...
for beam_energy in beam_energies:
    ...
    print(f"Plots for {beam_energy} will be saved in: {current_beam_energy_output_dir}")

    # Data Loading and Merging 
    print("\n--- Loading CSV Data for current beam energy ---")
    ...
    if mc_dis_df.empty or reco_df.empty:
        print("A required dataframe is empty, skipping this beam energy.")
        continue
    merged_df = pd.merge(mc_dis_df, reco_df, on='evt', how='inner')
    if merged_df.empty:
        print(f"Merged DataFrame is empty for {beam_energy} after merging.")
        continue
    
    reco_methods = sorted(list(set([col.split('_')[0] for col in reco_df.columns if '_' in col and col != 'evt' and not col.startswith('mc_')])))
    squared_w_methods = set()
    # ... W squaring logic can be added here if needed ...
    ...
    for var_key in kinematic_vars:
        ...
        for method in reco_methods:
            ...
            if reco_col in merged_df.columns:
                method_mask = pd.Series(True, index=merged_df.index)
                for cut_var in ['x', 'y']:
                    cut_col = f"{method}_{cut_var}"
                    if cut_col in merged_df.columns:
                        method_mask &= (merged_df[cut_col] >= 0) & (merged_df[cut_col] <= 1)
                df_plot = merged_df[method_mask]
                ...
                title_1d = f"{beam_energy}: {res_label} ({method})"
                title_2d = f"{beam_energy}: Resolution vs Truth ({method})"
    ...
    for method in reco_methods:
        ...
        y_regions = {'low_y': {'mask': merged_df[reco_y_col] <= 0.1, 'label': 'y <= 0.1'}, 'high_y': {'mask': merged_df[reco_y_col] > 0.1, 'label': 'y > 0.1'}}
        for region_name, region_info in y_regions.items():
            ...
            for var_key in ['x', 'q2']:
                ...
                if truth_col in df_plot.columns and reco_col in df_plot.columns:
                    ...
                    # Call for the linear plot
                    save_2dhist(temp_truth, resolution, truth_label_txt, res_label, title, filename, target_dir=current_method_output_dir, text_to_display=text)

                    # --- CHANGE APPLIED HERE ---
                    # If the variable is 'x', also create a version with a log x-axis
                    if var_key == 'x':
                        logx_title = title + "\n(Log x-axis)"
    ...
    # ... (ROOT file generation would go here) ...
